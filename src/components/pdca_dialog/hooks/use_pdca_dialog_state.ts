...
export const use_pdca_dialog_state = (initial_pdca: Pdca, current_user: { name?: string; email?: string } | null) => {
  ...
  const [comments_list, set_comments_list] = useState<PdcaComment[]>(initial_pdca.comentarios || []);
  const [history_events, set_history_events] = useState<PdcaHistoryEvent[]>(initial_pdca.historial || []);
  ...
  const [author_name, set_author_name] = useState<string>(initial_pdca.autor || current_user?.name || "Usuario");
  const [author_email, set_author_email] = useState<string>(initial_pdca.autorEmail || current_user?.email || "");
  const [assigned_users, set_assigned_users] = useState<{ name: string; email: string }[]>(initial_pdca.asignados || []);

  const [vpo_checkpoints, set_vpo_checkpoints] = useState<VpoCheckpointItem[]>(
    initial_pdca.vpoCheckpoints || DEFAULT_VPO_CHECKPOINTS
  );
  const [pareto_drill_downs, set_pareto_drill_downs] = useState<string[]>(initial_pdca.paretoDrillDowns || []);
  ...
  const [has_flavor_correlation, set_has_flavor_correlation] = useState<boolean>(initial_pdca.hasFlavorCorrelation || false);
  const [flavor_correlation_data, set_flavor_correlation_data] = useState<any>(initial_pdca.flavorCorrelationData || null);
  ...
  const [ishikawas, set_ishikawas] = useState<IshikawaItem[]>(() => {
    ...
    return [{
      ...
      causes: initial_pdca.ishikawaCauses || { machine: [], method: [], material: [], manpower: [], measurement: [], environment: [] },
  ...
  const [completed_phases, set_completed_phases] = useState<Set<string>>(new Set(initial_pdca.completedPhases || []));
  const [completed_steps, set_completed_steps] = useState<Set<string>>(new Set(initial_pdca.completedSteps || []));
  const [definition_goal, set_definition_goal] = useState<DefinicionMeta>(initial_pdca.definicionMeta || DEFAULT_DEFINICION_META);
  const [team_members, set_team_members] = useState<string[]>(initial_pdca.equipo || []);
  const [participants_data, set_participants_data] = useState<ParticipantesData>(initial_pdca.participantes || DEFAULT_PARTICIPANTES);

  return {
    active_tab, set_active_tab,
    title_value, set_title_value,
    area_value, set_area_value,
    problem_value, set_problem_value,
    root_cause_value, set_root_cause_value,
    action_items, set_action_items,
    comments_list, set_comments_list,
    history_events, set_history_events,
    bottom_tab, set_bottom_tab,
    deadline_date, set_deadline_date,
    author_name, set_author_name,
    author_email, set_author_email,
    assigned_users, set_assigned_users,
    vpo_checkpoints, set_vpo_checkpoints,
    ...
    kpi_nodes, kpi_edges, handle_kpi_change,
    completed_phases, set_completed_phases,
    completed_steps, set_completed_steps,
    definition_goal, set_definition_goal,
    team_members, set_team_members,
    participants_data, set_participants_data,
  };
};
